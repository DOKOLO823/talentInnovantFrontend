  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const resProfil = await apiFetch(`/entreprise/stats-globales`, {
          method: "GET",
        });
        const resChallenges = await apiFetch(
          "/entreprise/meschallenges/en-cours",
          { method: "GET" },
        );
        ...
        if (resChallenges?.statut === 200) {
          const cleanedChallenges = resChallenges?.challenges_en_cours?.map(
            (c: any) => {
              ...
              try {
                ...
                console.error("Erreur parse récompenses", e);
              }
              ...
              return {
                ...
                categories:
                  c?.domaines && c.domaines?.length > 0
                    ...
                    : ["Général"],
                entrepriseNom:
                  resProfil.data?.entreprise?.nom || "Votre Entreprise",
                entrepriseLogo:
                  apifile + "/" + resProfil.data?.entreprise?.user?.pp || null,
