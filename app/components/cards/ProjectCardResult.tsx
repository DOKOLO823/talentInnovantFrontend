...
import {
  MoreVertical, Edit, Trash2, Flag, MessageCircle,
  Maximize2, Trophy, Flame, ChevronDown, ChevronUp,
  X, UserRound, Mail, Phone, FileText, FileSpreadsheet, File,
  PencilLine, Loader2, AlertTriangle,
  Check,
  Copy,
  ExternalLink,
  Share2,
...
function ScoreModal({ isOpen, onClose, postId, currentNote, onUpdate, user, onUpdateCommentJury, currentCommentJury }: any) {
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden">
        <div className="bg-gray-50 px-6 py-4 border-b flex items-center justify-between">
            <div className="flex items-center gap-3">
                <Image src={apifile+'/' + user?.pp} width={35} height={35} alt="pp" className="rounded-full aspect-square object-cover border" />
                <div className="overflow-hidden">
                    <p className="font-bold text-xs truncate uppercase leading-tight line-clamp-1">{user?.talent?.nom?.length>20 ? user?.talent?.nom?.slice(0,20)+'...' : user?.talent?.nom || "Participant"}</p>
                    <p className="text-[10px] text-gray-500 font-medium truncate">{user?.talent?.profession?.length>30 ? user?.talent?.profession?.slice(0,30)+'...' : user?.talent?.profession || "Participant"}</p>
                </div>
            </div>
            <button onClick={onClose} className="p-2 hover:bg-gray-200 rounded-full transition-colors"><X size={18}/></button>
        </div>
        <div className="p-6 space-y-5">
          <h3 className="font-black text-lg text-gray-900 tracking-tight">Modifier la note</h3>
          <div className="space-y-4">
            <div>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Note du jury</label>
              <input type="number" value={note} onChange={(e) => setNote(e.target.value)} className="w-full mt-1 p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:border-orange-500 focus:bg-white outline-none font-black text-2xl" placeholder="00" min="0" max="20" />
            </div>
            <div>
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Commentaire</label>
              <textarea value={comment} onChange={(e) => setComment(e.target.value)} className="w-full mt-1 p-4 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:border-orange-500 focus:bg-white outline-none text-sm h-28 resize-none" placeholder="Avis..." />
            </div>
            <button onClick={handleSubmit} disabled={loading} className="w-full py-4 bg-orange-600 text-white rounded-2xl font-black shadow-lg shadow-orange-200 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:bg-gray-300">
              {loading ? <Loader2 className="animate-spin" size={20} /> : "VALIDER LA NOTE"}
            </button>
          </div>
        </div>
      </motion.div>
...
export default function ProjectCardResult({ project, challenge }: any) {
  ...
  const [localNote, setLocalNote] = useState(challenge?.typeevaluation?.type=='vote' ? project?.score : project?.notefinale);
  const [localCommentJury, setLocalCommentJury]=useState(project?.commentairejury[0]?.commentairejury ? project?.commentairejury[0]?.commentairejury : '')
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/challenge/post/delete/${project.id}`, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Accept": "application/json"
      ...
      if (data.statut === 200) {
        ...
        toast.error(data.message || "Impossible de supprimer le post", { id: deletingToast });
      }
    } catch (error) {
      toast.error("Erreur de connexion lors de la suppression", { id: deletingToast });
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="relative bg-white rounded-2xl shadow-sm border w-full max-w-xl mx-auto flex flex-col mb-6 overflow-hidden transition-all">
      ...
      <div className="flex justify-between items-center p-4 flex-shrink-0">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => router.push(`/profil-talent/${project?.user?.id}`)}>
          ...
          <div>
            <p className="font-bold text-gray-900 leading-tight text-sm hover:text-orange-600 transition-colors first-letter:uppercase">{(project?.user?.talent?.nom?.length>18 ? project?.user?.talent?.nom.substring(0, 18) + "..." : project?.user?.talent?.nom) || "Talent"}</p>
            <p className="text-[11px] text-gray-500 font-medium first-letter:uppercase">{(project?.user?.talent?.profession?.length>25 ? project?.user?.talent?.profession.substring(0, 25) + "..." : project?.user?.talent?.profession) || "Participant"}</p>
          </div>
        </div>
        
        <div className="flex items-center gap-1 relative" ref={dropdownRef}>
          {isOwner && (
            <button onClick={() => setShowContact(true)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-full transition-colors">
              <UserRound size={19} />
            </button>
          )}
  ...
  <div className="grid grid-cols-2 gap-2 text-[11px] font-bold">
    ...
    <div className="flex items-center gap-2 bg-white border border-gray-100 p-1.5 rounded-xl shadow-sm whitespace-nowrap">
      ...
      <div className="flex flex-col overflow-hidden">
        <span className="text-[9px] text-gray-500 font-medium leading-none mb-1">Rang final</span>
        <span className="text-gray-900 leading-none truncate text-[12px]">{project?.rang || '-'}</span>
      </div>
    </div>
    ...
    <div className="flex items-center justify-between bg-white border border-gray-100 p-1.5 rounded-xl shadow-sm whitespace-nowrap">
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col">
          <span className="text-[9px] text-gray-500 font-medium leading-none mb-1">{challenge.typeevaluation['type']=='vote' ? 'Score' : 'Note'} finale</span>
          <span className="text-gray-900 leading-none text-[12px]">{Number(localNote || 0).toFixed(2)} <span className="text-[9px]">{challenge.typeevaluation['type']=='vote' ? '' : '/20'}</span></span>
        </div>
      </div>

      {/* Bouton d'édition intégré à la card */}
      ...
      <div className="px-4 mt-5 space-y-6 overflow-y-auto max-h-[200px] scrollbar-thin scrollbar-thumb-gray-200 transition-all duration-300 pb-2">
        {responses.map((r: any, idx: number) => (
          <div key={r?.id} className={`group ${!showAllFields && idx > 1 ? 'hidden' : 'block'}`}>
            <p className="font-bold text-sm text-gray-950 mb-1.5 first-letter:uppercase">
              {r?.challenge_field?.label || "Information"}
            </p>
            <div className="text-gray-800">
              {r?.challenge_field?.type == "text" || r?.challenge_field?.type === "option" ? (
      ...
      {responses.length > 2 && (
        ...
        >
          {showAllFields ? (
            ...
            <><ChevronDown size={16} /> Voir les {responses.length - 2} autres champs</>
          )}
        </button>
      )}
  ...
  <div className="flex items-center gap-6">
    ...
    <div className="flex flex-col items-center justify-center text-center">
      <span className="text-sm font-black text-gray-900 leading-none">{project?.like}</span>
      <span className="text-[9px] font-bold text-gray-400 first-letter:uppercase tracking-tight mt-0.5">Vote(s)</span>
    </div>

    {/* Avis / Commentaires */}
    <button onClick={handleNavigateToReel} className="flex items-center gap-1 text-gray-600 hover:text-orange-700 transition group">
      <MessageCircle size={19} className="text-gray-400 group-hover:text-orange-600" />
      <div className="flex flex-col items-center justify-center text-center">
        <span className="text-sm font-black leading-none text-gray-900">{project?.commentaires_count}</span>
        <span className="text-[9px] font-bold text-gray-400 first-letter:uppercase tracking-tight mt-0.5">Avis</span>
      </div>
    </button>

    {/* Partages - AJOUTÉ ICI */}
    <button className="flex items-center gap-1 text-gray-600 hover:text-orange-700 transition group">
      <Share2 size={19} className="text-gray-400 group-hover:text-orange-600" />
      <div className="flex flex-col items-center justify-center text-center">
        <span className="text-sm font-black leading-none text-gray-900">{project?.partage}</span>
        <span className="text-[9px] font-bold text-gray-400 first-letter:uppercase tracking-tight mt-0.5">Partage(s)</span>
      </div>
    </button>
  </div>
  ...
  >
    <Maximize2 size={18} />
      ...
      <AnimatePresence>
        {showContact && (
          <ContactModal isOpen={showContact} onClose={() => setShowContact(false)} user={project?.user} />
        )}
        {showEditScore && (
          <ScoreModal 
