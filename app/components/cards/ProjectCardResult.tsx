...
function EditScoreModal({ isOpen, onClose, project, onSave }: any) {
  ...
  const handleValidate = () => {
    ...
    alert(`Note modifiée à ${val} pour le projet ID ${project.id}`);
  };
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <div className="mb-6">
          ...
          <input 
            ...
            className={`w-full p-4 rounded-xl border-2 outline-none transition-all text-xl font-bold text-center ${error ? 'border-red-500 bg-red-50' : 'border-gray-100 focus:border-orange-500'}`}
            placeholder="0.00"
          />
        ...
        <button 
          ...
          className="w-full py-4 bg-orange-700 text-white rounded-2xl font-bold shadow-lg shadow-orange-700/20 active:scale-95 transition-transform"
        >
...
export default function ProjectCardResult({ project, challenge }: any) {
  ...
  const handleSaveScore = (projectId: any, score: number) => {
    console.log(`Confirmation : Modification du projet ID ${projectId} avec la note ${score}`);
    // Ici votre logique API pour mettre à jour la note
  ...
  return (
    <div className="relative bg-white rounded-2xl shadow border w-full max-w-xl mx-auto flex flex-col mb-6">
      ...
      {/* RANG / SCORE + BOUTON MODIFIER */}
      <div className="px-4 flex items-center gap-6 text-gray-500 font-bold tracking-tight">
        ...
        <div className="flex items-center gap-2">
            ...
            <button 
                ...
                className="p-1.5 bg-gray-100 text-orange-700 hover:text-orange-500 hover:bg-orange-100 hover:scale-1.5 rounded-lg transition-all"
      ...
      {/* NOUVEAU MODAL DE MODIFICATION DE NOTE */}
      <EditScoreModal 
