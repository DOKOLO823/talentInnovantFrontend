...
import {
  ...
  Trophy, Flame, ChevronDown, ChevronUp, X, MessageSquare, 
  Facebook, AlertTriangle, Eye,
  Maximize2
} from "lucide-react";
import { motion } from "framer-motion";
import EvaluateProjectModal from "../modals/EvaluateProjectModal";

/* Composants auxiliaires fournis par vous restés inchangés */
...
export default function ProjectCardToEvaluate({ project, challenge }: any) {
  ...
  const handleNavigateToReel = () => {
    const query = new URLSearchParams({
      ...
      resultatdisponible: challenge?.resultatdisponible?.toString() || "non"
    }).toString();
  ...
  return (
    <div className="relative bg-white rounded-2xl shadow border w-full max-w-xl mx-auto flex flex-col mb-6">
      ...
      <div className="flex justify-between p-4">
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => router.push('/profil-talent/1')}>
      ...
      {/* RANG / SCORE */}
      <div className="px-4 flex gap-6 text-gray-500 font-bold tracking-tight">
        ...
        <span className="flex items-center gap-1 text-xs"><Flame size={14} className="text-orange-600" /> Note finale : <b className="text-gray-900">{project.notefinale ?? ''}</b></span>
      </div>

      {/* CONTENT - Garde l'overflow-y si showAllFields est vrai */}
      ...
      {/* BOUTON EVALUER (AJOUTÉ ICI) */}
      <div className="px-4 pb-4">
        <button
          onClick={() => setOpenEvalModal(true)}
          className="w-full flex items-center justify-center gap-2 bg-orange-700 text-white py-3 rounded-xl font-bold hover:bg-orange-800 transition shadow-lg shadow-orange-700/20 active:scale-95"
        >
          <Eye size={18} /> Évaluer le projet
        </button>
      </div>
      ...
      <div className="border-t px-4 py-3 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-gray-50/50 rounded-b-2xl">
        <button className="text-sm font-medium text-gray-700">{project.votesCount || 0} vote(s)</button>
      ...
      {openEvalModal && (
        <EvaluateProjectModal initialProjectId={project.id} onClose={() => setOpenEvalModal(false)} />
      )}
