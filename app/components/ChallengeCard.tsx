...
export default function ChallengeCard({ challenge }: { challenge: Challenge }) {
  ...
  const delayInfo = useMemo(() => {
    ...
    if (diffStart > 0) {
      return { text: `Début dans ${daysStart} jour${daysStart > 1 ? "s" : ""}`, color: "text-orange-700" };
    } else if (diffEnd > 0) {
      return { text: `Plus que ${daysEnd} jour${daysEnd > 1 ? "s" : ""} avant la fin`, color: "text-orange-700" };
    } else {
      const pastDays = Math.abs(daysEnd);
      return { text: `Terminé il y a ${pastDays} jour${pastDays > 1 ? "s" : ""}`, color: "text-gray-500" };
    }
    ...
    <>
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 w-full max-w-xs flex flex-col">
        ...
        <div className="relative h-32 w-full group overflow-hidden">
          ...
          <div className={`absolute inset-0 z-10 transition-all duration-500 ease-in-out ${openRewards ? "bg-black/90 backdrop-blur-md" : "bg-transparent"}`}>
            ...
            >
              <div className="flex items-center justify-between bg-black/60 backdrop-blur-2xl border border-white/10 px-3 py-2 rounded-lg transition-all hover:bg-black/60">
                ...
                {openRewards ? <ChevronUp className="w-4 h-4 text-white shrink-0" /> : <ChevronDown className="w-4 h-4 text-white shrink-0" />}
              </div>
            </div>
            : ''}
            <div className={`px-2 pb-3 overflow-y-auto transition-all duration-500 ${openRewards ? "opacity-100 h-[calc(100%-55px)]" : "opacity-0 h-0 invisible"}`}>
              <ul className="">
                {challenge?.rewards?.map((reward, index) => (
                  <li key={index} className="text-white text-[11px] flex items-start gap-2 bg-white/5 px-2 py-1 border  mb-2 border-gray-100 border-b-1 rounded">
          ...
          <div className={`absolute inset-0 z-30 bg-black/80 backdrop-blur-sm p-4 flex flex-col justify-center space-y-2 text-white text-xs transition-all duration-300 ${showDetails ? "opacity-100" : "opacity-0 invisible"}`}>
            <button onClick={() => setShowDetails(false)} className="absolute top-2 right-2 text-white/70 hover:text-white"><X className="w-5 h-5" /></button>
            <p className="flex items-center gap-2"><Calendar className="w-4 h-4 text-orange-400" /><span className="text-white">Début du challenge : {formatDate(challenge.startDate)}</span></p>
            <p className="flex items-center gap-2"><Calendar className="w-4 h-4 text-orange-400" /><span className="text-white">Fin du challenge : {formatDate(challenge.endDate)}</span></p>
            <p className="flex items-center gap-2"><Clock className="w-4 h-4 text-orange-400" /><span className="text-white">Fin des inscriptions : {formatDate(challenge.inscriptionEnd)}</span></p>
          </div>
        </div>
        ...
        <div className="p-4 space-y-3 flex-1 flex flex-col">
          ...
          <button
            onClick={() => router.push(`/profil-entreprise/${challenge?.entrepriseId}`)}
            className="flex items-center gap-2 text-xs text-gray-500 hover:text-orange-600 transition text-left"
          >
           <span> Publié par :</span>
            <img src={challenge.entrepriseLogo  ? challenge.entrepriseLogo : "../assets/images/ppe.png"} alt="Entreprise" className="w-4 h-4 rounded-full object-cover" />
            <span className="font-medium truncate">{challenge?.entrepriseNom?.length > 23 ? `${challenge?.entrepriseNom?.substring(0, 23)}...` : challenge?.entrepriseNom}</span>
          </button>
          ...
          <div className="flex flex-row justify-start items-center overflow-x-auto gap-2 w-full scrollbar-hide">
            {challenge.categories.map((cat) => (
              <div key={cat} className="text-xs font-medium px-3 py-1 rounded-full bg-orange-50 text-orange-700 border border-orange-100 whitespace-nowrap flex-shrink-0">
                {cat}
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1 text-sm border-b pb-2">
            {challenge?.participants>0 && <><Users className="w-4 h-4 text-orange-600" />
            <span className="font-bold text-orange-700">{challenge?.participants}</span>
      ...
      <ExternalRedirectModal 
        ...
        url={challenge.site}
      />
      <ProjectSubmissionModal isOpen={showSubmissionModal} onClose={() => setShowSubmissionModal(false)} challenge={challenge} />
    </>
  );
}