...
import { X, Trophy, ArrowRight, CheckCircle, Loader2, ChevronLeft, ChevronRight, FileText, File, EyeOff, FileSearch, Eye, Download, Maximize2 } from "lucide-react";
...
export default function EvaluateProjectModal({ initialProjectId, projectsList, onClose, onProjectEvaluated }: { initialProjectId: any, projectsList: any[], onClose: () => void, onProjectEvaluated: (id: number) => void }) {
  const [localProjects] = useState([...projectsList]);
  const initialIdx = localProjects.findIndex((p: any) => p.id === initialProjectId);
  ...
  const renderResponse = (resp: any) => {
    ...
    if (type === "text" || type === "textarea" || type === "option") {
        return (
            <div className="mb-4 p-4 bg-white border border-slate-100 rounded-2xl">
                <p className="text-[10px] font-black uppercase text-orange-600 mb-1 tracking-widest">{label}</p>
    ...
    return (
        <div className="mb-6">
            <p className="text-[10px] font-black text-orange-600 uppercase mb-2 ml-1">{label}</p>
            
            {isVideo ? (
                <video controls className="rounded-2xl w-full aspect-video bg-black shadow-sm">
                    <source src={fullUrl} />
                </video>
            ) : isImage ? (
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between px-1">
                        ...
                        <div className="flex items-center gap-1">
                            <button 
                                onClick={() => setIsFullscreenPreview({ url: fullUrl, label: label })}
                ...
                <a href={fullUrl} target="_blank" rel="noreferrer" className="flex items-center justify-between p-4 bg-white rounded-2xl border border-slate-200">
                    <span className="font-bold text-xs text-slate-700 truncate mr-2">{label}</span>
      ...
      >
        <div className="flex justify-between items-center px-4 py-3 md:px-8 md:py-5 bg-white border-b sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <div className="bg-orange-100 p-2 rounded-lg text-orange-600 md:block hidden"><Trophy size={18} /></div>
            <div>
              <h2 className="font-black text-slate-800 text-sm md:text-lg leading-none">Évaluation</h2>
              <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">Projet {currentIndex + 1}/{localProjects.length}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex bg-slate-100 rounded-lg p-1">
              <button disabled={currentIndex === 0} onClick={() => { setDirection(-1); setCurrentIndex(prev => prev - 1); }} className="p-1.5 hover:bg-white rounded-md disabled:opacity-20 transition-all"><ChevronLeft size={18} /></button>
              <button disabled={currentIndex === localProjects.length - 1} onClick={() => { setDirection(1); setCurrentIndex(prev => prev + 1); }} className="p-1.5 hover:bg-white rounded-md disabled:opacity-20 transition-all"><ChevronRight size={18} /></button>
            </div>
            <button onClick={onClose} className="p-2 bg-slate-100 hover:bg-red-50 hover:text-red-500 rounded-full transition-all ml-1"><X size={20} /></button>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden">
            ...
            <AnimatePresence mode="wait" custom={direction}>
              ...
              >
                <div className="flex items-center gap-3 p-4 rounded-2xl bg-white border border-slate-100 mb-6 shadow-sm">
                  <Image src={`${apifile}/${project?.user?.pp}`} width={48} height={48} alt="pp" className="rounded-xl h-12 w-12 object-cover border-2 border-slate-50" />
                  <div className="flex-1 min-w-0">
                    <p className="font-black text-slate-800 text-sm truncate">{project?.user?.talent?.nom || project?.user?.name}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase truncate">{project?.user?.talent?.profession || "Participant"}</p>
                  </div>
                </div>
                ...
                <div className="pb-24 md:pb-0">
                  ...
                  <div className="md:hidden mt-8 pt-8 border-t border-slate-200 space-y-6">
                     <div className="text-center">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Votre Note (sur 20)</p>
                        <input
                          type="number"
                          value={note}
          ...
          <div className="hidden md:flex w-[350px] bg-white border-l p-8 flex-col justify-between">
            <div className="space-y-8">
              <div className="text-center">
                <span className="inline-block px-3 py-1 bg-orange-50 text-orange-700 rounded-full text-[10px] font-black uppercase tracking-widest mb-4">Note Finale (sur 20)</span>
                <input
                  type="number"
                  value={note}
                  onChange={(e) => setNote(e.target.value)}
                  ...
                  placeholder="/20"
                />
                {errors.note && <p className="text-red-500 text-[10px] font-bold mt-2">{errors.note}</p>}
              </div>
            ...
            >
              {loading ? <Loader2 className="animate-spin" /> : (
                isFinalInModal ? <><CheckCircle size={20} /> TERMINER</> : <><ArrowRight size={20} /> NOTER & SUIVANT</>
              )}
            </button>
          </div>
        </div>
        ...
        <div className="md:hidden p-4 bg-white border-t sticky bottom-0 z-20 shadow-lg">
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={18} /> : (
              isFinalInModal ? "TERMINER L'ÉVALUATION" : "NOTER ET CONTINUER"
            )}
          </button>
        </div>
      </motion.div>
