...
import { 
  Loader2, Trash2, User, FileText, File, 
  Maximize2, Download, X, ExternalLink, AlertCircle 
} from "lucide-react";
...
export default function SubmissionDetailClient({ postId }: { postId: string }) {
  ...
  const fetchPostDetails = async () => {
    try {
      const res = await apiFetch(`/challenge/post/details/${postId}`, { method: "GET" });
      if (res.statut === 200) {
        setData(res.post);
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const res = await apiFetch(`/challenge/post/delete/${postId}`, { method: "GET" });
      if (res.statut == 200) {
        router.back(); 
  ...
  if (loading) return (
    <div className="h-screen w-full flex flex-col items-center justify-center gap-4 bg-white">
      ...
      <p className="text-slate-500 font-medium animate-pulse text-sm">Chargement de votre soumission...</p>
    </div>
  );
  ...
  if (!user) return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-slate-50 text-center">
      <div className="max-w-md bg-white p-8 rounded-[32px] shadow-xl border border-slate-100">
        ...
        <h2 className="text-xl font-black text-slate-800 mb-2">Accès restreint</h2>
        <p className="text-slate-500 text-[15px] mb-8">Vous devez être connecté pour voir les détails de cette soumission.</p>
        <div className="flex flex-col gap-3">
          <Link href="/auth/login" className="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold hover:bg-black transition-all">Se connecter</Link>
          <Link href="/auth/register-talent" className="w-full border-2 border-slate-200 text-slate-600 py-4 rounded-2xl font-bold hover:bg-slate-50 transition-all">S'inscrire</Link>
        </div>
      </div>
    </div>
  );
  ...
  if (data && data.user_id !== user.id && user.role !== 'admin') return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-slate-50 text-center">
      <div className="max-w-md bg-white p-8 rounded-[32px] shadow-xl border border-slate-100">
        <h2 className="text-xl font-black text-slate-800 mb-2">Oups !</h2>
        <p className="text-slate-500 text-[15px]">Cette soumission ne vous appartient pas.</p>
        <button onClick={() => router.back()} className="mt-6 text-orange-600 font-bold underline">Retourner en arrière</button>
      </div>
    </div>
  );
...
//   on verifie si le poste exite 
 if (!data) return (
  ...
  const isDeadlinePassed = data?.challenge?.datefininscription ? new Date(data.challenge.datefininscription) < new Date() : false;
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 w-full flex flex-row justify-center items-center">
      ...
      <BackButton />
        ...
        <Link href={`/challenge/${data?.challenge?.id}`} className="block mb-8 bg-white p-4 mx-1 border border-slate-100 shadow-sm hover:shadow-md transition-all">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 w-full">
              <div className="relative w-12 h-12 rounded-full bg-slate-100 border border-slate-50">
                <img 
                  src={data?.challenge?.photo ? `${apifile}/${data?.challenge?.photo}` : '../assets/images/award.jpg'} 
              ...
              <div className="w-8/9">
                <p className="text-[10px] font-black text-orange-600 tracking-widest mb-0.5">Challenge correspondant :</p>
                <h3 className="font-bold text-slate-900 text-sm line-clamp-2">{data?.challenge?.titre}</h3>
        ...
        <div className="flex items-center justify-between px-6 relative -top-2 mb-7">
          <div className="flex items-center gap-4">
            <h1 className="text-xl font-black text-slate-900 tracking-tight">Ma soumission</h1>
          </div>
          <button 
        ...
        <div className="flex items-center gap-4 mb-2 px-2 mx-6">
              <img src={data?.user?.pp ? `${apifile}/${data.user.pp}` : '../assets/images/pp2.png'} alt="Avatar" className="object-cover h-12 w-12" />
          ...
          {!isDeadlinePassed && data?.challenge?.datefininscription && (
            <div className="mt-12 p-6 rounded-[32px] text-center">
              ...
              <p className=" text-xs font-medium leading-relaxed">
                <span className="font-bold underline">NB</span> :
                Les soumissions des autres participants seront visibles dès la fin des inscriptions soit le : <br/>
                <span className="text-orange-700 font-bold">
                  {new Date(data.challenge.datefininscription).toLocaleString('fr-FR', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric',
                    hour: '2-digit',
                ...
                {data?.challenge?.typeevaluation_id!=2 && ' (Date à laquelle commencent les votes de projets)'}
              </p>
            </div>
          )}
        </div>
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="text-xl font-black text-slate-900 mb-2">Supprimer le projet ?</h3>
              <p className="text-slate-500 text-sm mb-8">Cette action est irréversible. Vos points liés à cette soumission seront déduits.</p>
              ...
              <div className="flex flex-col gap-3">
                ...
                >
                  {isDeleting ? <Loader2 className="animate-spin" size={20} /> : "Oui, supprimer"}
                </button>
                <button 
                  disabled={isDeleting}
