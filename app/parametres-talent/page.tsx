...
export default function ParametresTalent() {
  ...
  const handleToggleEmail = async () => {
    ...
    setToggling(true);
    try {
      const res = await apiFetch("/parametres-talent/receiveemail", { method: "GET" });
      if (res.statut === 200) {
  ...
  if (loading) return (
    <div className="h-screen w-full flex flex-col items-center justify-center gap-4 bg-white">
      ...
      <p className="text-slate-500 font-medium animate-pulse text-sm">Chargement de vos réglages...</p>
    </div>
  );
  ...
  if (!user) return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-slate-50">
      <div className="max-w-md bg-white p-8 rounded-[32px] shadow-xl border border-slate-100 text-center">
        ...
        <h2 className="text-xl font-black text-slate-800 mb-2">Accès refusé</h2>
        <p className="text-slate-500 mb-6">Veuillez vous connecter pour modifier vos paramètres.</p>
        <button onClick={() => window.location.href='/auth/login'} className="w-full bg-orange-600 text-white py-4 rounded-2xl font-bold">Connexion</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 pb-20 w-full flex flex-col items-center">
      ...
      <div className="w-full md:max-w-2xl pt-8 px-4">
        ...
        <header className="mb-10">
          ...
          <p className="text-slate-500 font-medium">Gérez vos préférences et notifications</p>
        </header>
        ...
        <div className="space-y-6">
          ...
          <section>
            <div className="flex items-center gap-2 mb-4 px-2">
                ...
                <h2 className="text-xs font-black uppercase tracking-widest">Notifications</h2>
            </div>

            <div className="bg-white rounded-[28px] border border-slate-100 shadow-sm overflow-hidden">
              <div className="p-6 flex items-center justify-between group transition-all">
                <div className="flex flex-col items-start gap-4">
                  <div className={`p-3 rounded-2xl transition-colors ${receiveEmail ? 'bg-orange-100 text-orange-600' : 'bg-slate-100 text-slate-400'}`}>
                    <Mail size={14} />
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-900">Challenges par mail</h3>
                    <p className="text-xs text-slate-500 font-medium leading-relaxed max-w-[220px] md:max-w-xs">
                      Être alerté dès qu'un nouveau challenge correspondant à mon profil est publié.
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  >
                    {toggling ? (
                      ...
                      receiveEmail && <CheckCircle2 className="text-orange-600" size={14} />
                    )}
                  </motion.span>
                </button>
              </div>
