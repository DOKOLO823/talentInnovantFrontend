...
useEffect(() => {
  const recordVisit = async () => {
    ...
    if (currentUser?.statut == 'entreprise' && userId && currentUser?.id != userId) {
      try {
    ...
    try {
      ...
      if (res?.statut === 200) {
        const transformedUser = {
          ...
          bio: res.user.bio || "",
          avatar: res.user.pp ? `${apifile}/${res.user.pp}` : "../assets/images/pp2.png",
          cover: res.user.pc ? `${apifile}/${res.user.pc}` : "../assets/images/pc2.jpeg",
          skills: res.talent?.competence 
            ? (Array.isArray(res.talent.competence) ? res.talent.competence : res.talent.competence.split(',').map((s: string) => s.trim()))
            : [],
        ...
        const transformedProjects = res.posts_par_challenge?.flatMap((challengeGroup: any[]) => 
          challengeGroup.map((post: any) => ({
            ...
            challenge: {
              ...
              image: post.challenge?.photo ? `${apifile}/${post.challenge.photo}` : "/default-challenge.jpg",
  ...
  if (error || !userData) {
    return (
      <div className="w-full text-center py-20">
        ...
        <button onClick={fetchUserProfile} className="mt-4 bg-orange-700 text-white px-5 py-2 rounded-lg">Réessayer</button>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      ...
      <div className="w-full h-56 md:h-100 relative">
        <Image src={userData.cover} fill className="object-cover" alt="cover" priority />
      </div>

      {/* PHOTO + INFOS */}
      <div className="px-4 md:px-8 -mt-20 relative">
        <div className="flex flex-col items-center md:items-start gap-4">
          <div className="w-36 h-36 md:w-40 md:h-40 rounded-full border-4 border-white overflow-hidden shadow-xl cursor-pointer" onClick={() => setShowPP(true)}>
            <img src={userData?.avatar} className="object-cover w-full h-full" alt="avatar" />
          </div>
          ...
          <div className="flex gap-2 flex-wrap justify-center">
            {isOwner && (
              <Link href={`/profil-talent/edit/${userId}`} className="bg-orange-700 text-white px-5 py-2 rounded-lg shadow hover:bg-orange-800 transition">
                Modifier mon profil
              </Link>
            )}
      ...
      {showPP && <FullImageModal src={userData.avatar} onClose={() => setShowPP(false)} />}
        ...
        {["about", "projets", "portfolio", "cv"].map((tab) => (
          <button
            ...
            className={`pb-3 border-b-2 whitespace-nowrap ${
              activeTab === tab ? "border-orange-700 text-orange-700 font-semibold" : "border-transparent text-gray-600"
            }`}
          >
            {tab === "about" ? "À propos" : tab === "cv" ? "CV" : tab.charAt(0).toUpperCase() + tab.slice(1)}
          </button>
        ))}
      </div>
      ...
      <div className="px-4 md:px-8 mt-6 pb-14">
        ...
        {activeTab === "projets" && <TabsProjects projects={projectsData} userId={userId} onRefresh={fetchUserProfile} currenUser={currentUser} />}
        {activeTab === "portfolio" && <TabsPortfolio userId={userId} userData={userData} currentUser={currentUser} />}
        {activeTab === "cv" && <TabsCV userId={userId} currentUser={currentUser} />}
      </div>
      ...
      <AnimatePresence>
        {showContactModal && (
          <div className="fixed inset-0 z-[700] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowContactModal(false)} className="absolute inset-0 bg-black/60 backdrop-blur-md" />
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="relative bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl text-center">
              <button onClick={() => setShowContactModal(false)} className="absolute top-4 right-4 p-2 text-gray-400 hover:text-black transition-colors"><X /></button>
              ...
              <div className="mb-6">
                ...
                <h3 className="text-xl font-bold line-clamp-1">Contacter {userData.name}</h3>
                <p className="text-gray-500 text-sm line-clamp-1">{userData.profession}</p>
              </div>
              
              <div className="space-y-3">
                <a href={`mailto:${userData.email}`} className="flex items-center justify-center gap-3 w-full py-4 bg-orange-50 text-orange-700 rounded-2xl font-bold hover:bg-orange-100 transition-all border border-orange-100">
                  <Mail size={20} /> {userData.email}
                </a>
                <a href={`tel:${userData.telephone}`} className="flex items-center justify-center gap-3 w-full py-4 bg-gray-50 text-gray-700 rounded-2xl font-bold hover:bg-gray-100 transition-all border border-gray-100">
