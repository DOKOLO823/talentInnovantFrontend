...
import { Users, Target, LayoutDashboard, TrendingUp, Loader2 } from 'lucide-react';
...
export default function ChallengeAnalytics({ challengeId }: { challengeId: number }) {
  ...
  if (error || !data) {
    return (
      <div className="w-full p-8 text-center bg-red-50 rounded-xl border border-red-100">
        <p className="text-red-600 font-medium">Impossible de charger les statistiques du challenge.</p>
        <button 
          onClick={fetchStats}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Participants</p>
        ...
        <div className="bg-white border border-slate-200 p-6 rounded-xl shadow-sm md:col-span-2">
          <div className="flex justify-between items-center mb-4">
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Taux de participation</p>
              <h3 className="text-2xl font-bold text-slate-800">{data.summary.taux_participation}%</h3>
            </div>
          ...
          <p className="text-[11px] text-slate-400 mt-2 font-medium">
            {data.summary.total_participants_actifs} talents ont rejoint sur {data.summary.total_talents_domaine} talents identifiés dans les domaines de ce challenge
          </p>
        </div>
      </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data.registrationData}>
                ...
                <Line 
                  ...
                  stroke={ORANGE_700} 
                  strokeWidth={3}
                  dot={{ r: 6, fill: ORANGE_700, strokeWidth: 2, stroke: '#fff' }}
                  activeDot={{ r: 8 }}
                />
              </LineChart>
            </ResponsiveContainer>
