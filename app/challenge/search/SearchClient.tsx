...
export default function SearchClient() {
  ...
  const [categoryName, setCategoryName] = useState(searchParams.get("domaineName") || "");
    ...
    try {
      ...
      if (res && res.statut === 200) {
        const mappedData = res.data.map((c: any) => {
          ...
          const formatDateFull = (d: string) => d ? new Date(d).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }) : "---";
          ...
          return {
            ...
            endDate: formatDateFull(c.datefin),
  ...
  useEffect(() => {
    ...
    fetchChallenges(searchParams.get("domaine") || undefined, searchParams.get("lieu") || undefined);
  }, [searchParams]);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="py-16 max-w-7xl mx-auto px-4">
        <div className="text-xl font-bold text-gray-900 mb-8">
          {loading ? "Recherche..." : `${results.length} Résultat(s) pour `} {!loading && <span className="text-orange-700">{`${categoryName && categoryName || "Tous les domaines"} ${location && ': '+location || ": Partout"}`}</span>}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center text-gray-500 py-20 bg-white rounded-xl border border-dashed mb-12">Aucun résultat trouvé.</div>
        ) : (
