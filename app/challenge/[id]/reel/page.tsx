...
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_BASE_URL || "https://talentinnovant.com/api";

// AJOUT POUR REGLER L'ERREUR DE BUILD
export async function generateStaticParams() {
  try {
    // On récupère la liste des challenges pour que Next.js puisse pré-générer les pages
    const res = await fetch(`${API_BASE_URL}/challenges`);
    const data = await res.json();

    // On s'adapte à la structure de ta réponse API (data.data ou data)
    ...
    return []; // Retourne un tableau vide pour ne pas bloquer tout le build
  }
}
...
async function fetchChallengeDetails(challengeId: string) {
  try {
    const res = await fetch(
      `${API_BASE_URL}/challenge/details/${challengeId}`,
      {
        next: { revalidate: 60 },
      },
    );
