    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-3 bg-black/70 backdrop-blur-md">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="p-4 text-center">
          ...
          <h3 className="text-2xl font-black text-gray-900 mb-3">Challenge Externe</h3>
          <p className="text-gray-600 text-sm leading-relaxed mb-8">
            Ce challenge se déroule en dehors de <strong>TALENT INNOVANT</strong>. 
            Vous allez être redirigé vers la plateforme du challenge pour soumettre votre candidature.
          </p>
...
function PrivateResultCard({ project, rank, challenge }: any) {
  ...
  return (
    <Link href={'/profil-talent/' + project.user?.id} className="bg-white rounded-xl p-4 shadow-md border-l-4 border-orange-600">
      <div className="flex justify-between items-start">
        <div className="flex gap-3">
          <div className="w-12 h-12 rounded-full bg-gray-200 overflow-hidden relative">
            <Image src={project.user?.pp ? `${apifile}/${project.user.pp}` : "/avatar.png"} alt="User" fill className="object-cover" />
          </div>
          <div>
            <p className="font-bold text-gray-900 uppercase text-xs">{project.user?.talent?.nom?.length > 15 ? project.user?.talent?.nom?.substring(0, 15) + "..." : project.user?.talent?.nom || "Talent"}</p>
  ...
  (challenge?.typeevaluation?.type === 'vote' 
    ...
    : Number(project?.notefinale || 0).toFixed(2))
     ...
     {(challenge?.typeevaluation?.type !='vote' && project.commentaire_jury) &&  <div className="mt-4 p-3 bg-gray-50 rounded-lg">
        <p className="text-xs font-bold text-gray-500 mb-1 flex items-center gap-1"><MessageSquare size={12}/> Commentaire du jury :</p>
        <p className="text-sm text-gray-600 italic">
          {isExpanded ? project.commentaire_jury : `${project.commentaire_jury?.substring(0, 100)}...`}
        </p>
        {project.commentaire_jury?.length > 100 && (
          <button onClick={() => setIsExpanded(!isExpanded)} className="text-orange-600 text-xs font-bold mt-1">
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden animate-in fade-in zoom-in duration-300 relative">
        <div className="p-6 text-center">
          ...
          <p className="text-gray-600 text-sm mb-6">
            Vous devez être connecté pour participer à ce challenge et soumettre votre projet.
          </p>
          <div className="space-y-3">
            <a href="/auth/login" className="flex items-center justify-center gap-2 w-full py-3 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-orange-200">
              Se connecter
            </a>
            <a href="/auth/register-talent" className="flex items-center justify-center gap-2 w-full py-3 bg-white border-2 border-gray-100 hover:border-orange-200 text-gray-700 font-bold rounded-xl transition-all">
...
function ShareModal({ isOpen, onClose, challengeId, onShareSuccess }: any) {
  ...
  if (!isOpen) return null;
  const shareUrl = typeof window !== "undefined" ? `${window.location.origin}/challenge/${challengeId}` : "";
    ...
    <div className="fixed inset-0 bg-black/60 z-[300] flex items-center justify-center p-4 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl border">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-black text-xl text-gray-900">Partager le challenge</h3>
          <button onClick={onClose} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors"><X size={20}/></button>
        </div>
        <div className="mb-5">
          <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2 block">Message (optionnel)</label>
          <textarea 
        ...
        <div className="grid grid-cols-3 gap-3">
          <button onClick={() => handleShareAction('whatsapp')} className="flex flex-col items-center gap-2 group">
            <div className="p-4 bg-[#25D366] text-white rounded-2xl w-full flex justify-center shadow-lg group-hover:scale-105 transition-transform"><MessageCircle fill="currentColor" /></div>
          ...
          <button onClick={() => handleShareAction('facebook')} className="flex flex-col items-center gap-2 group">
            <div className="p-4 bg-[#1877F2] text-white rounded-2xl w-full flex justify-center shadow-lg group-hover:scale-105 transition-transform"><Facebook fill="currentColor" /></div>
          ...
          <button onClick={() => handleShareAction('copy')} className="flex flex-col items-center gap-2 group">
            <div className="p-4 bg-gray-800 text-white rounded-2xl w-full flex justify-center shadow-lg group-hover:scale-105 transition-transform"><Copy /></div>
...
export default function ChallengeClient() {
      ...
      if (response.statut == 200) {
        ...
        setChallengeBestFormat({
           id: response?.data?.challenge.id,
              ...
              image: response?.data?.challenge.photo ? `${apifile}/${response?.data?.challenge.photo}` : "/assets/images/innov.jpg",
        })
 ...
 const handleLike = async () => {
  ...
  const handleShareSuccess = async () => {
    ...
    } catch (e) { console.error("Erreur increment partage"); }
  };
  ...
  if (loading) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-orange-700" size={48} /></div>;
  if (!challenge) return <div className="p-20 text-center relative top-4">Désolé, ce challenge a été supprimé. </div>;

  const tabs = [
    ((isOwner || isJury) && challenge?.site?.toLowerCase()=='talent innovant') && { id: "stats", label: "Statistiques" },
    ((isOwner || isJury) && challenge?.site?.toLowerCase()=='talent innovant') && { id: "evaluate", label: "Évaluer les projets" },
    { id: "overview", label: "Aperçu" },
    challenge?.site?.toLowerCase()=='talent innovant' && { id: "projects", label: "Projets" },
    { id: "rules", label: "Règles" },
    { id: "criteria", label: "Critères" },
    { id: "rewards", label: "Récompenses" },
    challenge?.site?.toLowerCase()=='talent innovant' && { id: "participants", label: "Participants" },
    challenge?.site?.toLowerCase()=='talent innovant' && { id: "results", label: "Résultats" },
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="relative w-full h-70 md:h-80 rounded-b-2xl overflow-hidden shadow mt-6">
        <Image src={challenge.photo ? `${apifile}/${challenge.photo}` : '../assets/images/innov.jpg'} alt="Cover" fill className="object-cover" />
        <div className="absolute inset-0 bg-black/50" />
        <div className="absolute bottom-6 left-6 text-white">
          <h1 className="text-2xl md:text-4xl font-bold line-clamp-6">{challenge.titre}</h1>
      ...
      {challenge?.site?.toLowerCase()!='talent innovant' && 
        <div className="px-4 text-xs text-orange-700 leading-relaxed mt-4">
          <span className="font-semibold underline">NB:</span> Les inscriptions et le déroulement de ce challenge se font hors de <span className="font-bold">TALENT INNOVANT</span>.
        </div>
      }

      <div className="flex flex-row flex-wrap items-center gap-4 px-6 mt-4">

       {((isOwner || isJury) && (challenge?.site?.toLowerCase()=='talent innovant')) &&  <button 
...
>
  <Eye size={18} /> Voir le formulaire de participation
       ...
       {( (challenge?.site?.toLowerCase()=='talent innovant') && (!isOwner && !isJury) ) &&  <button 
 ...
 {(challenge?.site?.toLowerCase()!='talent innovant') &&  <button 
          onClick={handleParticipate}
          className="bg-orange-700 hover:bg-orange-800 text-white md:px-6 md:py-2 px-3 py-2 rounded-xl shadow flex items-center gap-2 transition-all active:scale-95"
        >
  ...
  className={`px-4 py-2 rounded-lg flex items-center justify-center gap-2 border transition-all active:scale-95 ${
    hasLiked ? "bg-orange-50 border-orange-700 text-orange-700" : "bg-white text-gray-700 hover:bg-gray-100"
  }`}
>
  {loaderLikeChallenge ? (
    <Loader2 size={18} className="animate-spin text-orange-700" /> 
  ) : (
    <Heart size={18} className={hasLiked ? "fill-orange-700 text-orange-700" : ""} />
  )}
  <span className="font-bold">{likeursIds?.length}</span>
</button>

        <button onClick={() => setShowShareModal(true)} className="bg-white border px-4 py-2 rounded-lg flex items-center gap-2 text-gray-700 hover:bg-gray-100 transition-all active:scale-95">
          <Share2 size={18} /> Partager
        </button>

        {isOwner && (
          <div className="ml-auto flex gap-3">
            <Link href={`/challenge/edit/${challenge?.id}`} className="p-2 text-orange-700 hover:bg-orange-100 rounded-lg flex items-center gap-1 border border-orange-200">
              <Pencil size={18} /> <span className="hidden md:inline">Modifier</span>
            </Link>
            <button onClick={() => setShowDeleteModal(true)} className="p-2 bg-orange-700 hover:bg-orange-600 text-white rounded-lg flex items-center gap-1 border border-red-200">
              <Trash size={18} /> <span className="hidden md:inline">Supprimer</span>
            </button>
          </div>
        )}
      </div>
      ...
      <div className="mt-8 px-3 max-w-7xl mx-auto">
        {activeTab === "stats" && <ChallengeAnalytics challengeId={challenge?.id} />}
        {activeTab === "evaluate" && <EvaluateProjectsSection challenge={challenge} />}
        {activeTab === "overview" && <OverviewSection challenge={challenge} />}
        {activeTab === "projects" && <ProjectsSection challenge={challenge} currentUser={currentUser} isOwner={isOwner} />}
        {activeTab === "rules" && <ListSection title="Règles du challenge" data={challenge?.principe} />}
        {activeTab === "criteria" && <ListSection title="Critères d'évaluation" data={challenge?.critereevaluation} />}
        {activeTab === "rewards" && <ListSection title="Récompenses" data={challenge?.recompense} />}
        {activeTab === "participants" && <ParticipantsSection challengeId={challengeId} currentUser={currentUser} />}
        {activeTab === "results" && <ResultsSection challenge={challenge} isOwner={isOwner} currentUser={currentUser} />}
      </div>
      ...
      <AnimatePresence>
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 bg-black/60 z-[300] flex items-center justify-center p-4 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl border text-center">
              <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <Trash className="text-red-600" size={32} />
              </div>
              <h3 className="font-black text-xl text-gray-900 mb-2">Confirmation</h3>
              <p className="text-gray-600 mb-6">Êtes-vous sûr de vouloir supprimer ce challenge ? Cette action est irréversible.</p>
              <div className="flex gap-3">
                <button onClick={() => setShowDeleteModal(false)} className="flex-1 py-3 border rounded-xl font-bold hover:bg-gray-50 transition">Annuler</button>
                <button 
...
function EvaluateProjectsSection({ challenge }: any) {
  const [activeSubTab, setActiveSubTab] = useState(challenge?.typeevaluation?.type?.toLowerCase()!='vote' ? "a_evaluer" : 'provisoire');
...
const maintenant = new Date().getTime();
  ...
  const fetchEvalData = async () => {
    ...
    try {
      ...
      const endpoint = activeSubTab === "a_evaluer" 
      ...
      if (activeSubTab === "a_evaluer") {
        ...
        setProjects(evaluated.sort((a: any, b: any) => b.notefinale - a.notefinale));
      }
    } catch (e) { 
      toast.error("Erreur de chargement"); 
    } finally {
  ...
  const handlePublishResults = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/challenge/publier-resultats/${challenge?.id}`, {
        method: "GET",
        headers: {
  ...
  const renderNombreAEvaluer = () => {
    try {
      ...
      const data = typeof challenge?.nombregagnant === 'string' 
      ...
      return (
        <span className="text-gray-700 text-sm">
          <span>Le jury procédera à l'évaluation des <span className="font-bold">{data[0]}</span> premiers projets (uniquement) figurant sur cette liste.</span>
        </span>
      );

    } catch (e) {
      // En cas d'erreur de parsing ou format inattendu
      return <span className="text-gray-700 text-sm">{challenge?.nombregagnant || ""}</span>;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex gap-2 p-1 bg-gray-200 rounded-xl w-fit">
          {challenge?.typeevaluation?.type!='vote' && <button 
        ...
        {activeSubTab === "provisoire" && (
          <button
            ...
            disabled={isPublishing || challenge?.resultatdisponible == 1 || (dateFin > maintenant && challenge?.typeevaluation?.type?.toLowerCase()=='vote')}
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-black text-xs transition-all ${
              challenge?.resultatdisponible == 1 || ((challenge?.typeevaluation?.type?.toLowerCase()=='hybride' || challenge?.typeevaluation?.type?.toLowerCase()=='vote') && dateFin > maintenant)
              ? 'bg-gray-100 text-gray-400 border border-gray-200' 
              : 'bg-orange-700 text-white hover:bg-orange-600 shadow-lg shadow-blue-100'
            }`}
          >
            {isPublishing ? <Loader2 className="animate-spin" size={16} /> : (challenge?.resultatdisponible == 1 ? <CheckCircle size={16} /> : <Send size={16} />)}
            {challenge?.resultatdisponible == 1 ? "RÉSULTATS DÉJÀ PUBLIÉS" : "PUBLIER LES RÉSULTATS"}
          </button>
        )}
      </div>

     {(challenge?.typeevaluation?.type?.toLowerCase()=='hybride' && dateFin > maintenant && activeSubTab === 'a_evaluer') && <span className=" text-sm"> NB: Les notations des projets commencent le <span className="font-bold">{formatDate(challenge?.datefin)}</span> (date de fin des votes de projets) </span>}

     {(challenge?.typeevaluation?.type?.toLowerCase()=='hybride' && dateFin < maintenant && activeSubTab === 'a_evaluer') && <div className=" mt-3"> {renderNombreAEvaluer()} </div>}

      {/* GRILLE DE PROJETS */}
      {loading ? (
        <div className="py-20 flex justify-center"><Loader2 className="animate-spin text-orange-700" size={40} /></div>
      ) : (
...
function OverviewSection({ challenge }: any) {
  ...
  const getJuryName = () => {
    ...
    if (!jury) return 'Le jury sera communiqué prochainement par l\'organisateur.';
  ...
  const renderGagnantsDescription = () => {
    try {
        ...
        return (
          <div className="space-y-1">
            <p className="text-gray-700 text-sm">
              <span className="font-bold">{data[0]}</span> finalistes pour la première phase (vote)
            </p>
            <p className="text-gray-700 text-sm">
              <span className="font-bold">{data[1]}</span> gagnants finaux délibérés par un jury d'experts
            </p>
          </div>
        );
      }
      ...
      // En cas d'erreur de parsing ou format inattendu
      return <span className="text-gray-700 text-sm">{challenge?.nombregagnant || "Non défini"}</span>;
    }
  };
    ...
    <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-100 md:space-y-5">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-700 leading-relaxed">{challenge?.objectif || 'Atteindre les meilleurs résultats selon les critères définis.'}</p>
        </div>
      </div>

      {challenge?.site?.toLowerCase()=='talent innovant' &&   <div className="mt-8">
          <span className="text-black flex flex-row font-extrabold flex items-center gap-2 mb-3 first-letter:uppercase tracking-tight">
            <Star size={18} className="text-orange-600" /> Type d'évaluation des projets : 
          </span>
          <span className="text-orange-700 relative -top-2 text-sm uppercase font-bold">{challenge?.typeevaluation?.type}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-12">
          ...
          <div className="space-y-4">
            <div className="flex flex-col">
              <span className="text-black font-bold text-sm first-letter:uppercase">Début du challenge</span>
              <span className="text-gray-700 text-sm">{datelancement}</span>
            </div>
           {challenge?.typeevaluation?.type?.toLowerCase()!='hybride' &&  <div className="flex flex-col">
              <span className="text-black font-bold text-sm first-letter:uppercase">Fin du challenge</span>
              <span className="text-gray-700 text-sm">{datefin}</span>
            </div>}
            <div className="flex flex-col">
              <span className="text-black font-bold text-sm first-letter:uppercase">Fin des soumissions de projets {(challenge?.typeevaluation?.type?.toLowerCase()=='vote' || challenge?.typeevaluation?.type?.toLowerCase()=='hybride') && ' et début des votes'} {challenge?.typeevaluation?.type?.toLowerCase()=='jury' && ' et début des évaluations des projets'} </span>
              <span className="text-gray-700 text-sm">{datefininscription}</span>
            </div>
            {challenge?.typeevaluation?.type?.toLowerCase()=='hybride' &&  <div className="flex flex-col">
              <span className="text-black font-bold text-sm first-letter:uppercase">Fin des votes et début des évaluations</span>
              <span className="text-gray-700 text-sm">{datefin}</span>
            </div>}
            <div className="flex flex-col">
              <span className="text-black font-bold text-sm first-letter:uppercase">Lieu</span>
              <span className="text-gray-700 text-sm capitalize">{challenge?.lieu}</span>
            </div>
          </div>

          {/* Colonne 2 : Accès, Participation & Gagnants */}
          <div className="space-y-4">
           {(challenge?.site?.toLocaleLowerCase()=='talent innovant') &&  <div className="flex flex-col">
              <span className="text-black font-bold text-sm first-letter:uppercase">Site</span>
              <span className="text-gray-700 text-sm">{challenge?.site || 'Plateforme Talent Innovant'}</span>
            </div>}
            <div className="flex flex-col">
              <span className="text-black font-bold text-sm first-letter:uppercase">Soumissions autorisées</span>
              <span className="text-gray-700 text-sm">{challenge?.nombrecontribution || 1} projet(s) au max par participant</span>
            </div>
           <div className="flex flex-col">
  <span className="text-black font-bold text-sm first-letter:uppercase mb-1">Nombre de gagnants finaux</span>
  {renderGagnantsDescription()}
          ...
          <div className="space-y-6">
           <div className="flex flex-col">
  <span className="text-black font-bold text-sm mb-2 first-letter:uppercase">Public cible</span>
  <ul className="space-y-1">
    {(Array.isArray(challenge?.publiccible) 
      ...
      <li key={index} className="text-gray-700 text-sm flex items-center gap-2">
        <div className="w-1 h-1 bg-orange-700 rounded-full" /> {item}
      </li>
    ))}
  </ul>
</div>
            ...
            <div className="flex flex-col">
              <span className="text-black font-bold text-sm mb-2 first-letter:uppercase">Domaines concernés</span>
              <ul className="space-y-1">
                {challenge?.domaines?.map((domaine: any, index: number) => (
                  <li key={index} className="text-gray-700 text-sm flex items-center gap-2">
                    <div className="w-1 h-1 bg-orange-700 rounded-full" /> {domaine.nom}
                  </li>
                ))}
                {(!challenge?.domaines || challenge?.domaines?.length === 0) && (
      ...
      <div className="space-y-4 mt-5">
            <div className="flex flex-col">
              <span className="text-black font-bold text-sm first-letter:uppercase">Détails supplémentaires</span>
              <span className="text-gray-700 text-sm">{challenge?.details || ''}</span>
            </div>
      </div>
      }
      ...
      <div className="pt-7 border-t border-gray-50 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          <h3 className="text-black font-extrabold flex items-center gap-2 mb-3 first-letter:uppercase tracking-tight">
            <ShieldCheck size={18} className="text-orange-600" /> Portée : {challenge.portee?.portee}
          </h3>
...
function ProjectsSection({ challenge, currentUser, isOwner }: any) {
  ...
  useEffect(() => {
    const fetchProj = async () => {
      try {
        const jsonAll = currentUser ? await apiFetch(`/challenge/posts/${challenge?.id}`) : { posts: [], top_posts: [] };
        let jsonMy = { data: [] };
        if (currentUser) {
  ...
  const isRegistrationClosed = new Date() > new Date(challenge?.datefininscription);
  const formattedDate = new Date(challenge?.datefininscription).toLocaleDateString('fr-FR', {
    ...
    <div className="space-y-6">
      ...
      {currentUser && !canView && (
        <div className="p-10 bg-gray-50 rounded-2xl text-center border-2 border-dashed border-orange-700">
          ...
          <p className="font-bold text-black">Ce challenge est privé. Seul le créateur peut visualiser les projets soumis.</p>
        </div>
      )}

      {/* MESSAGE D'ATTENTE : S'affiche UNIQUEMENT si l'utilisateur n'est pas owner ET n'a aucun projet soumis */}
      {currentUser && canView && !isRegistrationClosed && !isOwner && !hasMyProjects && (
        <div className="p-10 bg-gray-50 rounded-2xl text-center border-2 border-dashed border-orange-700">
          <Clock className="mx-auto mb-3 text-orange-700" />
          <p className="font-bold text-black">Les projets seront visibles à partir du {formattedDate}{challenge?.typeevaluation?.type !='jury' && ', Date à laquelle commencent les votes.'}</p>
        </div>
      )}
        ...
        <>
          ...
          {!isRegistrationClosed && !isOwner && hasMyProjects && (
             <p className="text-sm font-bold text-orange-700 italic mb-4">
              Note : Vous visualisez vos projets soumis. Les projets des autres participants seront visibles le {formattedDate}{challenge?.typeevaluation?.type !='jury' && ', Date à laquelle commencent les votes.'}
            </p>
          )}
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 scroll-smooth">
            {hasMyProjects && (
              <button onClick={() => setActiveSubTab("mes")} className={`px-5 py-2 rounded-full font-bold text-sm whitespace-nowrap transition-colors ${activeSubTab === "mes" ? "bg-orange-700 text-white" : "bg-white border text-gray-600"}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(() => {
              ...
              if (activeSubTab === "mes") currentData = data?.my_posts;
                ...
                currentData = activeSubTab === "populaires" ? data?.top_posts : data?.posts;
              }

              if (currentData?.length === 0) {
                return <div className="col-span-full text-center py-10 text-gray-500 font-bold text-sm">Aucun projet à afficher.</div>;
              }
...
function ListSection({ title, data }: { title: string, data: any }) {
  ...
  const getIcon = () => {
    ...
    if (t.includes('récompense')) return <Trophy size={20} className="text-orange-700" />;
    if (t.includes('critère')) return <Pencil size={20} className="text-orange-700" />;
    if (t.includes('règle')) return <ListOrdered size={20} className="text-orange-700" />;
...
function ParticipantsSection({ challengeId, currentUser }: { challengeId: string, currentUser: any }) {
  ...
  return (
    <div className="w-full py-4">
      ...
      {!loading && currentUser && (<div className="flex items-center gap-2 mb-6 px-1 text-gray-800">
  ...
  <div className="p-10 bg-orange-50 rounded-2xl text-center border-2 border-dashed border-orange-200">
    <Lock className="mx-auto mb-3 text-orange-600" size={32} />
    <p className="font-bold text-black mb-6">Connectez-vous pour voir les participants.</p>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-shrink-0">
                    ...
                    <Image
                      src={p.user?.pp ? `${apifile}/${p.user.pp}` : '../assets/images/pp2.png'}
                      alt="pp"
                      width={44}
                      height={44}
                ...
                <div className="flex flex-col overflow-hidden">
                  <span className="font-bold text-gray-900 capitalize truncate">
                    {p.talent?.nom?.length>20 ? p.talent?.nom.substring(0, 20) + "..." : p.talent?.nom || "Innovateur"}
                  </span>
                  <span className="text-[11px] text-gray-500 font-semibold uppercase tracking-tight">
                    {p.talent?.profession?.length>30 ? p.talent?.profession.substring(0, 30) + "..." : p.talent?.profession || "Talent"}
                  </span>
                </div>
              </div>
...
const limit = renderNombreGagnant()
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        ...
        if (challenge?.typeevaluation == "vote" || challenge?.typeevaluation?.type == "vote") {
  ...
  if ((challenge?.resultatdisponible != '1' && challenge?.typeevaluation?.type != 'vote') || (challenge?.typeevaluation?.type == 'vote' && challenge?.datefin > new Date().toISOString() && challenge?.resultatdisponible != '1')) {
    return (
  ...
  <div className="flex flex-row items-center gap-2 md:gap-4 overflow-x-auto no-scrollbar py-2 max-w-full px-3 bg-gray-100 rounded-xl">
    ...
    >
      <Trophy size={18} className={activeSubTab === 'winners' ? "text-white" : "text-orange-600"} />
    ...
    >
      <ListOrdered size={18} className={activeSubTab === 'rest' ? "text-white" : "text-orange-600"} />
      Reste du classement
    </button>
  </div>
</div>
...
{/* etre connecte pour voir les resultats */}
 {!currentUser && (
  <div className="p-10 bg-orange-50 rounded-2xl text-center border-2 border-dashed border-orange-200">
    <Lock className="mx-auto mb-3 text-orange-600" size={32} />
    <p className="font-bold text-black mb-6">Connectez-vous pour voir les résultats.</p>
    ...
    <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
      ...
      {(challenge?.portee?.portee == 'privee' && currentUser) && (
        <p className="text-center text-[12px] md:text-xs font-bold italic px-4">
          Note : Ce challenge est privé. Les projets ne sont visibles que par les administrateurs. Les talents ne voient que le classement par équipe.
        </p>
      )}
        ...
        {loading ? (
          ...
          (activeSubTab === "winners" ? winners : rest).map((p: any, idx) => {
            ...
            if (isPrivate && !isOwner) {
              return <PrivateResultCard key={p.id} project={p} rank={p.rang} challenge={challenge} />;
            }
      ...
      {/* Message vide */}
      {!loading && currentUser && (activeSubTab === "winners" ? winners : rest)?.length == 0 && (
