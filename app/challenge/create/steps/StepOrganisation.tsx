...
import { Info, MapPin, Users, Target, Search, Check, ChevronDown, User, Briefcase, Settings2, Shield, Globe, Link as LinkIcon } from "lucide-react";
...
export default function StepOrganisation({
  ...
  // Styles Enterprise
  const labelStyle = "block text-[11px] font-black uppercase tracking-wider text-slate-900 mb-2";
  ...
  const sectionTitle = "text-sm font-bold text-slate-900 mb-6 flex items-center gap-2 border-b border-slate-100 pb-4";

  const filteredUsers = users.filter((u) => {
    const name = u.statut === 'talent' ? `${u.talent?.nom} ${u.talent?.prenom}` : u.entreprise?.nom;
  ...
  const showGeo = data.lieu === "en presentiel" || data.lieu === "hybride";
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 py-4">
      <div className="border-b border-slate-200 pb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">Configurez la visibilité et le protocole d'évaluation.</p>
      </div>

      <div className="grid md:grid-cols-2 gap-6">
        {/* CONFIGURATION ACCÈS */}
        <section className={cardStyle}>
          <h3 className={sectionTitle}><Shield size={16} className="text-orange-700"/> Accès & Visibilité</h3>
          <div className="space-y-6">
            ...
            {showGeo && (
              <div className="grid grid-cols-2 gap-3 p-3 bg-orange-50/50 rounded-lg border border-orange-100 animate-in fade-in zoom-in-95 duration-300">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-orange-800 uppercase">Région</label>
                  <select 
                    className={inputStyle}
                    value={data.region || ""}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-orange-800 uppercase">Ville</label>
                  <select 
                    className={inputStyle}
                    value={data.ville || ""}
                  ...
                  >
                    <option value="">Choisir...</option>
                    {data.region && CAMEROON_GEO[data.region].map(ville => (
            ...
            <div className="space-y-1">
              ...
              <select 
                className={inputStyle} 
                value={data.site === "talent innovant" || !data.site ? "talent innovant" : "hors talent innovant"} 
                onChange={(e) => {
                  const val = e.target.value;
                  update("site", val === "talent innovant" ? "talent innovant" : " ");
                }}
              >
                <option value="talent innovant">Plateforme Talent Innovant (In-app)</option>
                <option value="hors talent innovant">Formulaire Externe (Lien URL)</option>
              </select>
            </div>

            {showUrlInput && (
              <div className="space-y-1 animate-in fade-in slide-in-from-top-2 duration-300">
                <label className={labelStyle}>Lien de participation (URL)</label>
            ...
            <div className="space-y-1">
              ...
              <div className="grid grid-cols-2 gap-2 bg-slate-50 border border-slate-200 p-1 rounded-md">
                {[ { id: 1, label: "Public" }, { id: 2, label: "Privé" }].map(p => (
                  <button
                    key={p.id}
                    type="button"
        ...
        {/* MODE D'ÉVALUATION */}
        <section className={cardStyle}>
          <h3 className={sectionTitle}><Users size={16} className="text-orange-700"/> Système de notation</h3>
          <div className="space-y-6">
            <div className="space-y-1">
                ...
                >
                  {selectedJury ? (
                    <div className="flex items-center gap-2 overflow-hidden">
                      <img 
                        src={selectedJury.pp ? `${apifile}/${selectedJury?.pp}` : "../assets/images/pp2.png"} 
                        className="w-5 h-5 rounded-full object-cover ring-1 ring-slate-200" 
                        alt=""
                      />
                      <span className="truncate font-bold text-xs">
                        {selectedJury.statut === 'talent' ? `${selectedJury.talent?.nom} ${selectedJury.talent?.prenom}` : selectedJury.entreprise?.nom}
                      </span>
                    </div>
                  ) : (
                    <span className="text-slate-400">Sélectionner un membre...</span>
                  )}
                  ...
                  <div className="absolute z-50 w-full mt-1 bg-white border border-slate-300 rounded-md shadow-xl overflow-hidden ring-1 ring-black/5">
                    ...
                    <div className="max-h-48 overflow-y-auto">
                      {filteredUsers.map((u) => (
                        <div 
                          key={u.id}
                          onClick={() => { update("jury_id", u.id); setIsOpen(false); }}
                        ...
                        >
                          <img src={u.pp ? `${apifile}/${u.pp}` : "../assets/images/pp2.png"} className="w-6 h-6 rounded-full object-cover" alt="" />
                          <div className="flex-1 min-w-0">
                            <p className="text-[11px] font-bold text-slate-900 line-clamp-1">{u.statut === 'talent' ? `${u.talent?.nom ? u.talent?.nom : ''} ${u.talent?.prenom ? u.talent?.prenom : ''}` : u.entreprise?.nom ? u.entreprise?.nom : ''}</p>
                            <p className="text-[9px] text-slate-400 uppercase font-black tracking-tighter italic line-clamp-1">{u.statut === 'talent' ? `${u.talent?.profession ? u.talent?.profession : ''}` : u.entreprise?.service ? u.entreprise?.service : ''}</p>
                          </div>
                          {data.jury_id === u.id && <Check size={14} className="text-orange-700" />}
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <section className={cardStyle}>
        <div className="grid md:grid-cols-2 gap-10">
          <div className="space-y-2">
            <label className={labelStyle}>Quotas de lauréats (Nombre de gagnants)</label>
            {data.typeevaluation === "Hybride" ? (
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">Préselections (Vote)</p>
                  <input type="number" className={inputStyle} value={data.nombregagnant?.[0] || ""} onChange={(e) => {
                ...
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">Nombre gagnants Finaux</p>
                  <input type="number" className={inputStyle} value={data.nombregagnant?.[1] || ""} onChange={(e) => {
                    const copy = [...(data.nombregagnant || ["", ""])];
              ...
              <input type="number" className={inputStyle} placeholder="Ex: 3" value={data.nombregagnant?.[0] || ""} onChange={(e) => update("nombregagnant", [e.target.value])} />
            )}
          </div>

          <div className="space-y-2">
            <label className={labelStyle}>Nombre de soumission par participant</label>
            <input 
              type="number" 
              className={inputStyle} 
              value={data.nombrecontribution || 1} 
              onChange={(e) => update("nombrecontribution", parseInt(e.target.value))} 
            />
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Nb: nombre maximum de projets par participant</p>
          </div>
        </div>
      </section>
      ...
      <div className="flex justify-between items-center pt-6 border-t border-slate-200">
        <button type="button" onClick={onBack} className="text-[12px] font-bold text-slate-400 hover:text-slate-900 uppercase tracking-widest transition-colors">
          Précédent
        </button>
        <button 
          type="button" 
