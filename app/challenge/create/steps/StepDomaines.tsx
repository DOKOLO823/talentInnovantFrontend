...
import { Check, LayoutGrid, Info, Target } from "lucide-react";
        ...
        <div className="flex gap-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed uppercase font-medium">
              Les domaines sélectionnés déterminent quels talents seront notifiés. 
              Un ciblage précis augmente le taux de participation de <span className="text-white font-bold">40%</span>.
            </p>
          </div>
        </div>
      </div>
        ...
        {domaines.map((domaine) => {
          ...
          return (
            <button
              ...
              className={`
                relative text-left p-4 rounded-md border transition-all duration-200 group
                ${
                  isSelected
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <h3 className={`font-bold text-[13px] leading-tight pr-6 ${isSelected ? "text-orange-900" : "text-slate-900"}`}>
                  {domaine.nom}
                </h3>
                {isSelected && (
                  <div className="bg-orange-700 rounded-full p-0.5">
                    <Check className="w-3 h-3 text-white" />
                  </div>
                )}
              </div>
              <p className={`text-[11px] leading-relaxed line-clamp-2 font-medium ${isSelected ? "text-orange-800/70" : "text-slate-500"}`}>
                {domaine.description}
              </p>
            </button>
          );
          ...
          {data.domaines?.length > 0 && (
            <div className="flex items-center gap-2 px-3 py-1 bg-slate-100 rounded-full border border-slate-200">
              ...
              <span className="text-[10px] font-black text-slate-600 uppercase tracking-tighter">
                {data.domaines.length} Sélectionné{data.domaines.length > 1 ? "s" : ""}
              </span>
            </div>
          )}
        </div>
