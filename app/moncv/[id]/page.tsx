...
import { Loader2, AlertCircle, ChevronLeft, ExternalLink, Lock, Building2 } from "lucide-react";
  ...
  if (loading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-50">
        ...
        <p className="text-slate-500 font-medium animate-pulse uppercase tracking-widest text-xs">Chargement du CV...</p>
      </div>
    );
  }
  ...
  if (error || !cv || !checkVisibility()) {
    ...
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-white p-6 text-center">
        <div className={`p-6 rounded-full mb-6 ${isRestricted ? "bg-orange-50" : "bg-red-50"}`}>
          {isRestricted ? (
        ...
        <div className="flex flex-col gap-3">
            <Link href="/" className="bg-slate-900 text-white px-8 py-3 rounded-2xl font-bold hover:bg-black transition shadow-lg">
                Retour à l'accueil
            </Link>
    ...
    <div className="fixed inset-0 bg-slate-900 flex flex-col overflow-hidden">
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 z-50 shadow-sm">
        <div className="flex items-center gap-4 flex-1">
          <button 
            onClick={() => window.location.href='/profil-talent/'+cv?.talent?.user_id} 
          ...
          <div className="flex flex-col min-w-0">
            ...
            <div className="flex items-center gap-3">
                ...
                <span className="hidden md:flex items-center gap-1 text-[9px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md font-bold uppercase">
                    {cv.visibilite === 'entreprise' ? <Building2 size={10}/> : null}
                    Accès {cv.visibilite}
                </span>
            </div>
          </div>
