...
export default function HomePage() {
  ...
  const [loadingChallengesTermines, setLoadingChallengesTermines] =
  ...
  useEffect(() => {
    const fetchChallengesByTab = async () => {
      ...
      try {
        if (activeTab === "Challenges terminés") {
          ...
          const res = await apiFetch("/meschallenges/termines", {
            method: "GET",
          });
          if (res?.statut === 200) {
    ...
    if (
      activeTab !== "Mes Challenges" ||
      (activeTab === "Mes Challenges" && mesChallengesSubTab === "termines")
    ) {
  ...
  const getCurrentChallenges = () => {
    if (activeTab === "Mes Challenges") {
      return mesChallengesSubTab === "en_cours"
      ...
      <section className="bg-white px-6 py-10 md:py-16 border-b border-gray-50 overflow-hidden">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mt-6">
            <div
              onClick={() => navigateToTab("Mes Challenges", "en_cours")}
            ...
            >
              <div className="text-xl md:text-3xl font-extrabold text-gray-900 leading-none">
                {!initialLoadComplete ? (
                  ...
                  statsUser.mesChallengesEnCours
                )}
              </div>
            ...
            <div
              onClick={() => navigateToTab("Mes Challenges", "termines")}
              className="relative bg-white rounded-xl px-5 pt-5 pb-2 shadow-sm border-t-4 border-orange-700 text-left cursor-pointer transition-all duration-300 hover:shadow-2xl group"
            >
              <div className="text-xl md:text-3xl font-extrabold text-gray-900 leading-none">
                {!initialLoadComplete ? (
                  ...
                  statsUser.mesChallengesTermines
                )}
              </div>
              <p className="text-sm font-medium text-gray-500 mt-4 flex flex-col items-center gap-1">
            ...
            <div
              ...
              className="relative bg-white rounded-xl px-5 pt-5 pb-1 shadow-sm border-t-4 border-orange-700 text-left cursor-pointer transition-all duration-300 hover:shadow-2xl group"
            >
              <div className="text-xl md:text-3xl font-extrabold text-gray-900 leading-none">
                {!initialLoadComplete ? (
                  <Loader2 className="w-6 h-6 animate-spin text-orange-700" />
                ) : (
                  statsUser.challengesTotalEnCours
                )}
              </div>
              <p className="text-sm font-medium text-gray-500 mt-4 flex flex-col items-center gap-1">
                Total challenges en cours <ChevronDown size={14} />
              </p>
              <div className="absolute top-3 right-4 p-2 rounded-full bg-orange-100">
            ...
            >
              <div className="text-xl md:text-3xl font-extrabold text-gray-900 leading-none">
                {!initialLoadComplete ? (
                  ...
                  statsUser.challengesAvenir
                )}
              </div>
              <p className="text-sm font-medium text-gray-500 mt-4 flex flex-col items-center gap-1">
                Total challenges à venir <ChevronDown size={14} />
              </p>
              <div className="absolute top-3 right-4 p-2 rounded-full bg-orange-100">
      ...
      >
        ...
        {activeTab === "Mes Challenges" && (
          <div className="flex gap-2 px-5 py-3 shadow-sm">
            ...
            >
              En cours{" "}
              {initialLoadComplete && "(" + mesChallengesEnCours?.length + ")"}
            </button>
            <button
            ...
            >
              Terminés{" "}
              {loadingChallenges && mesChallengesSubTab === "termines" && (
                <Loader2 className="w-3 h-3 animate-spin" />
              )}{" "}
              {initialLoadComplete && ` (${statsUser?.mesChallengesTermines})`}
            </button>
          </div>
        )}
